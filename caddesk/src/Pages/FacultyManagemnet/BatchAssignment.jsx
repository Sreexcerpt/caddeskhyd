...
const BatchAssignment = () => {
    ...
    useEffect(() => {
        // Fetch batches to extract unique branches
        fetchBatches();
    ...
    const fetchBatches = async () => {
        ...
        try {
            ...
            // Extract unique branches from batch records
            const uniqueBranches = response.data.reduce((acc, batch) => {
                // Check if branch already exists in accumulator
                if (!acc.some(branch => branch.branchId === batch.branchId)) {
                    acc.push({
                        branchId: batch.branchId,
                        // You might want to add a method to get branch name if available
                        branchName: `Branch ${batch.branchId}` // Fallback name
                    });
                }
                return acc;
            }, []);
    ...
    const handleBranchChange = (branchId) => {
        // Filter batches for the selected branch
        const filteredBatches = batches.filter(batch => batch.branchId === branchId);
        ...
        setForm(prev => ({ 
            ...
            subjectCode: "", 
            facultyId: "", 
            schedule: weekdays.map(day => ({ day, timeSlot: "" })) 
        }));
        
        // Set filtered batches
    ...
    const handleBatchChange = async (batchId) => {
        ...
        if (batchId) {
            setLoading(true);
            try {
                // Find the selected batch to get its details
                const selectedBatch = batches.find(b => b.batchId === batchId);
                setSelectedBatchDetails(selectedBatch);
    ...
    const handleSubjectChange = async (subjectCode) => {
        ...
        if (subjectCode && selectedBatchDetails) {
            setLoading(true);
            try {
                // Fetch faculties who can teach this subject and are in the same branch
                const response = await axios.get(`http://localhost:8080/getFacultiesBySubjectAndBranch/${subjectCode}/${selectedBatchDetails.branchId}`);
                setFaculties(response.data);
                setLoading(false);
            } catch (err) {
                console.error("Error fetching faculties:", err);
                                ...
                                <div className="card-body">
                                    ...
                                    <div className="batch-assignment-container">
                                        ...
                                        <form onSubmit={handleSubmit}>
                                            ...
                                            <div className="col-md-3">
                                                <div className="form-group">
                                                    ...
                                                    <select
                                                        ...
                                                        value={form.batchId}
                                                        onChange={(e) => handleBatchChange(e.target.value)}
                                                        disabled={!form.branchId || loading}
                                                    >
                                                        <option value="">Select Batch</option>
                                                        {batches.map(batch => (
                                            ...
                                            {/* Rest of the existing dropdowns (Subject, Faculty) */}
                                            <div className="col-md-3">
                                                <div className="form-group">
                                                    <label>Subject:</label>
                                                    <select
